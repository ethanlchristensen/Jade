...
{
    auto play_test = dpp::slashcommand();
    ...
    auto filters = dpp::command_option(dpp::co_string, "filter", "The filter to apply to the audio.",
                                                      false);
    for (auto & [fst, snd] : TEST_FILTERS)
    {
        filters.add_choice(dpp::command_option_choice(fst, fst));
    }
...
void play_test_process(dpp::cluster &bot, const dpp::slashcommand_t& event, std::string& query, const std::string& filter, JadeQueue &queue)
{
    ...
    const SongInfo songInfo = getSongInfo(query);
    const SongRequest song{query, filter, event};
    const dpp::embed embed = NowPlayingEmbed(song, songInfo);
    const dpp::message now_playing_message(event.command.channel_id, embed);
