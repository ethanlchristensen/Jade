...
void loadSlashCommands(dpp::cluster &bot) {
    if (dpp::run_once<struct register_bot_commands>()) {
        bot.log(dpp::ll_info, "creating slash commands.");
...
void processSlashCommand(dpp::cluster &bot, const dpp::slashcommand_t &event, JadeQueue &songQueue, OllamaAPI &ollamaApi) {
    ...
    if (command == "nolan") {
        bot.log(dpp::ll_info, "/nolan called by " + event.command.usr.global_name + ".");
        nolan_process(bot, event);
    }
    else if (command == "clear") {
        bot.log(dpp::ll_info, "/clear called by " + event.command.usr.global_name + ".");
        clear_process(bot, event);
    }
    else if (command == "echo") {
        bot.log(dpp::ll_info, "/echo called by " + event.command.usr.global_name + ".");
        ...
        bot.log(dpp::ll_info, "/play called by " + event.command.usr.global_name + ".");
        ...
        bot.log(dpp::ll_info, "/playtest called by " + event.command.usr.global_name + ".");
        std::string music_query = std::get<std::string>(event.get_parameter("query_or_link"));
        std::variant filter_provided = event.get_parameter("filter");
        std::string filter;
        if (filter_provided.index() > 0) {
        ...
        bot.log(dpp::ll_info, "/join called by " + event.command.usr.global_name + ".");
        join_process(bot, event);
    }
    else if (command == "leave") {
        bot.log(dpp::ll_info, "/leave called by " + event.command.usr.global_name + ".");
        leave_process(bot, event);
    }
    else if (command == "pause") {
        bot.log(dpp::ll_info, "/pause called by " + event.command.usr.global_name + ".");
        pause_process(bot, event);
    }
    else if (command == "resume") {
        bot.log(dpp::ll_info, "/resume called by " + event.command.usr.global_name + ".");
        resume_process(bot, event);
    }
    else if (command == "skip") {
        bot.log(dpp::ll_info, "/skip called by " + event.command.usr.global_name + ".");
        skip_process(bot, event, songQueue);
    } else if (command == "chat") {
        bot.log(dpp::ll_info, "/chat called by " + event.command.usr.global_name + ".");
        ...
        if (std::holds_alternative<std::string>(event.get_parameter("model"))) {
            ...
            model = "Jade";
            bot.log(dpp::ll_info, "No valid 'model' provided; defaulting to 'Jade'.");
        }
        chat_process(bot, event, message, model, ollamaApi);
    } else if (command == "describe") {
        bot.log(dpp::ll_info, "/describe called by " + event.command.usr.global_name + ".");
        describe_process(bot, event, ollamaApi);
    } else if (command == "reply")
    {
        bot.log(dpp::ll_info, "/reply called by " + event.command.usr.global_name + ".");
